...
import { Vote, Users, Award, ArrowLeft, Download } from 'lucide-react';
...
const Results: React.FC = () => {
  ...
  const chartData = currentCategoryResults?.candidates.map((c: any) => ({
    name: c.full_name,
    votes: c.vote_count,
    value: c.vote_count // for pie chart
  })) || [];
  ...
  return (
    <PageWrapper
      ...
      subtitle={`Final outcome for ${election.title}`}
      actions={
    ...
    >
      <Link to="/voter/history" className="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-indigo-600 mb-6 transition-colors">
        <ArrowLeft size={16} />
        Back to History
      </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <StatsCard 
          ...
          value={currentCategoryResults?.total_votes || 0} 
          icon={Vote} 
          color="indigo" 
        />
        <StatsCard 
          title="Voter Turnout" 
          value={`${currentCategoryResults?.turnout_percentage || 0}%`} 
          icon={Users} 
          color="blue" 
        />
        <StatsCard 
          title="Leading Candidate" 
          value={winner?.full_name || 'N/A'} 
          icon={Award} 
          color="green" 
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Category Selection & Detailed List */}
              ...
              {currentCategoryResults?.candidates.map((c: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-indigo-600">{c.vote_count} votes</p>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">
                      {((c.vote_count / (currentCategoryResults.total_votes || 1)) * 100).toFixed(1)}% share
                    </p>
                  </div>
                </div>
              ))}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card">
            <h3 className="font-bold text-gray-900 mb-6">Vote Share Percentage</h3>
